...
import { Search, Plus, MoreVertical, Building2, MapPin, Globe, Phone, Mail, Clock, FileText } from 'lucide-react';
...
export default function Clients() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setFormData({ nombre_cliente: '', nombre_negocio: '', sector: 'otro', estado_relacion: 'lead', prioridad: 'media' });
    }
  };
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    ...
    if (res.ok) {
      setClients(clients.map(c => c.id === id ? { ...c, estado_relacion: newStatus } : c));
      if (selectedClient && selectedClient.id === id) {
        setSelectedClient({ ...selectedClient, estado_relacion: newStatus });
  ...
  const filteredClients = clients.filter(c => {
    const matchesSearch = c.nombre_cliente.toLowerCase().includes(search.toLowerCase()) ||
                ...
                <tr key={client.id} onClick={() => openClientDetails(client)} className="hover:bg-zinc-50 transition-colors cursor-pointer">
                  ...
                  <td className="px-6 py-4">
                    <select
                      ...
                      className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium capitalize border-0 focus:ring-2 focus:ring-zinc-900 cursor-pointer
                        ${client.estado_relacion === 'activo' ? 'bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20' : 
                          client.estado_relacion === 'lead' ? 'bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-600/20' : 
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium capitalize
                      ${client.prioridad === 'alta' ? 'bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/20' : 
                        client.prioridad === 'media' ? 'bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20' : 
                        'bg-zinc-100 text-zinc-700 ring-1 ring-inset ring-zinc-500/20'}`}>
                      {client.prioridad}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button 
                      onClick={(e) => { e.stopPropagation(); openClientDetails(client); }}
          ...
          <div className="space-y-6">
            <div className="flex items-start justify-between">
              ...
              <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium capitalize
                ${selectedClient.estado_relacion === 'activo' ? 'bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20' : 
                  selectedClient.estado_relacion === 'lead' ? 'bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-600/20' : 
                  'bg-zinc-100 text-zinc-700 ring-1 ring-inset ring-zinc-500/20'}`}>
